...
L = float(sys.argv[2]) ##5.0
obs_err = float(sys.argv[3]) ##1.0
# param1 = np.arange(0, 9)
# param2 = np.array([0.5, 0.8, 1.0, 1.2, 1.5])
nens = int(sys.argv[4]) ##20
# param1 = np.array([10, 20, 40, 80, 160])
F = float(sys.argv[5]) ##8.0
# ROI = 0
# inflate = 0.0
param2 = np.arange(5, 51, 5)
param1 = np.arange(1.0, 1.2, 0.02)
...
for i in range(param1.size):
  for j in range(param2.size):
    ...
    casename = filter_kind+"/dk20/L{:3.1f}_s{:3.1f}".format(L, obs_err)+"/N{}_F{}".format(nens, F)+"/ROI{}".format(param2[j])+"_inf{:4.2f}".format(param1[i])
...
# plt.colorbar(cs)
# ax = plt.subplot(122)
# cs = ax.contourf(ii, jj, CRa, np.arange(0, 2.05, 0.05), cmap='bwr')
# plt.colorbar(cs)
